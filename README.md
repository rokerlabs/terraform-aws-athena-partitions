...
| <a name="input_query_result"></a> [query\_result](#input\_query\_result) | Specify an existing (S3 bucket) query result location. If not set, the module will create an S3 bucket for the functions query results. | <pre>object({<br>    bucket_arn = string<br>    location   = string<br>  })</pre> | `null` | no |
| <a name="input_release"></a> [release](#input\_release) | Release/version of the Athena Partitions function code e.g. `v0.0.0`. | `string` | `"v1.0.2"` | no |
